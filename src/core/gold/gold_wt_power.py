...
def _build_summary(df: DataFrame, window: str = "day") -> DataFrame:
    ...
    Returns:
        DataFrame with columns: turbine_id, period, period_min_power, period_max_power, period_avg_power,
    ...
    return (
        ...
        .agg(
            round(min("power_output"), 2).alias("period_min_power"),
            round(max("power_output"), 2).alias("period_max_power"),
            round(avg("power_output"), 2).alias("period_avg_power"),
        )
        # Fleet benchmarks: average of each per-turbine metric across all turbines in the period
        .withColumn("fleet_avg_min_power", round(mean("period_min_power").over(fleet_window), 2))
        .withColumn("fleet_avg_max_power", round(mean("period_max_power").over(fleet_window), 2))
        .withColumn("fleet_avg_power", round(mean("period_avg_power").over(fleet_window), 2))
        .withColumn("window_type", lit(window))
    )


def _enrich_with_anomaly_flags(summary_df: DataFrame) -> DataFrame:
    """Flag turbines whose period_avg_power deviates more than 2 standard deviations from the fleet mean.

    Catches turbines that are significantly under- or over-performing relative to
    their peers at the same point in time.
...
if __name__ == "__main__":
    ...
    aggregate_wind_turbines(spark, "day")
